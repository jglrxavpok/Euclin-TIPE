...
object TestTypes {
    ...
    @JvmStatic fun main(args: Array<String>) {
        val sourceCode = javaClass.getResourceAsStream("/TestTypes.euclin").bufferedReader().use { it.readText() } // ferme le flux après
        val data = EuclinCompiler.compile(sourceCode, "TestTypes.euclin")

        FileOutputStream(File("runtime/tests", "TestTypes.class")).use {
            it.write(data)
            it.flush()
            it.close()
        }
